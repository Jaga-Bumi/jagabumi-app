<aside x-data="{ collapsed: false }" :class="collapsed ? 'w-16' : 'w-64'" class="fixed left-0 top-0 h-screen bg-sidebar border-r border-sidebar-border transition-all duration-300 z-40 flex flex-col">
  ...
  <nav class="flex-1 p-2 space-y-1 overflow-y-auto">
    ...
    <a href="{{ route('organization.profile') }}" 
      ...
      <span x-show="!collapsed">Profil Organisasi</span>
    </a>
    ...
    <a href="{{ route('organization.members.index') }}" 
      ...
      <span x-show="!collapsed">Manajemen Anggota</span>
    </a>
    @endif
    ...
    <a href="{{ route('organization.quests.index') }}" 
      ...
      <span x-show="!collapsed">Manajemen Quest</span>
    </a>

    {{-- Articles Management --}}
    <a href="{{ route('organization.articles.index') }}"
        :class="collapsed ? 'justify-center px-2' : 'px-3'"
        class="flex items-center gap-3 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 {{ request()->routeIs('organization.articles*') ? 'bg-primary text-primary-foreground shadow-glow' : 'hover:bg-muted' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {{-- Icon for Articles (e.g., a document or book) --}}
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5s3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18s-3.332.477-4.5 1.253" />
        </svg>
        <span x-show="!collapsed">Manajemen Artikel</span>
    </a>
    ...
    <a href="{{ route('organization.prizes') }}" 
      ...
      <span x-show="!collapsed">Pemberian Hadiah</span>
    </a>
  </nav>
  ...
  <div class="p-2 border-t border-sidebar-border space-y-1">
    <a href="{{ route('home') }}" 
      ...
      <span x-show="!collapsed">Balik ke Main Site</span>
    </a>
  </div>
</aside>
