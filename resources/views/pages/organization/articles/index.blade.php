...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="glass-card p-6 rounded-2xl">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Artikel Organisasi</h2>
                ...
                @foreach($articles as $article)
                    ...
                    <div class="flex items-center p-4 rounded-xl bg-muted/50 hover:bg-muted transition-colors border border-border/50">
                        ...
                        <div class="flex items-center gap-4 flex-1 min-w-0 pr-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-sm text-muted-foreground">
                                    Dibuat: {{ $article->created_at->format('M j, Y') }}
                                    @if ($article->user)
                                        <span class="ml-2">| Oleh {{ $article->user->name }}</span>
                                    @endif
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ...
                            {{-- Edit Link --}}
                            <a href="{{ route('organization.articles.edit', $article->id) }}" class="text-primary hover:text-primary/80 transition-colors p-2 rounded-full hover:bg-primary/10" title="Edit Artikel">
                            ...
                            {{-- Delete Form --}}
                            <form action="{{ route('organization.articles.destroy', $article->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?');">
                                @csrf
                                @method('DELETE')
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <p class="text-muted-foreground mb-4">Organisasi Anda belum membuat artikel.</p>
