    ...
    validateField(field) {
      ...
      if (field === 'title') {
        if (!this.formData.title || this.formData.title.trim().length === 0) {
          this.errors.title = 'Quest title is required';
        } else if (this.formData.title.length > 255) {
          this.errors.title = 'Quest title must not exceed 255 characters';
        }
      } else if (field === 'desc') {
        ...
        if (!this.formData.banner) {
          this.errors.banner = 'Quest banner image is required';
        }
      }
      // Step 2 validations
      else if (field === 'location_name') {
        if (!this.formData.location_name || this.formData.location_name.trim().length === 0) {
          this.errors.location_name = 'Location name is required';
        }
        ...
        if (this.formData.radius_meter && (isNaN(this.formData.radius_meter) || this.formData.radius_meter < 10)) {
          this.errors.radius_meter = 'Radius must be at least 10 meters';
        }
      }
      // Step 3 validations
      else if (field === 'registration_start_at') {
        if (!this.formData.registration_start_at) {
          this.errors.registration_start_at = 'Registration start date is required';
        }
      } else if (field === 'registration_end_at') {
        if (!this.formData.registration_end_at) {
          this.errors.registration_end_at = 'Registration end date is required';
        }
      } else if (field === 'quest_start_at') {
        if (!this.formData.quest_start_at) {
          this.errors.quest_start_at = 'Quest start date is required';
        }
      } else if (field === 'quest_end_at') {
        if (!this.formData.quest_end_at) {
          this.errors.quest_end_at = 'Quest end date is required';
        }
      } else if (field === 'judging_start_at') {
        if (!this.formData.judging_start_at) {
          this.errors.judging_start_at = 'Judging start date is required';
        }
      } else if (field === 'judging_end_at') {
        if (!this.formData.judging_end_at) {
          this.errors.judging_end_at = 'Judging end date is required';
        }
      } else if (field === 'prize_distribution_date') {
        if (!this.formData.prize_distribution_date) {
          this.errors.prize_distribution_date = 'Prize distribution date is required';
        }
        ...
        if (!this.formData.participant_limit || isNaN(this.formData.participant_limit) || this.formData.participant_limit < 1) {
          this.errors.participant_limit = 'Participant limit must be at least 1';
        }
      ...
      // Step 4 validations
        ...
        if (!this.formData.cert_name || this.formData.cert_name.trim().length === 0) {
          this.errors.cert_name = 'Certificate name is required';
        }
      } else if (field === 'cert_image') {
        if (!this.formData.cert_image) {
          this.errors.cert_image = 'Certificate image is required';
        }
      } else if (field === 'cert_description') {
        if (!this.formData.cert_description || this.formData.cert_description.trim().length === 0) {
          this.errors.cert_description = 'Certificate description is required';
        }
      }
    },
    ...
    <form action="{{ route('organization.quests.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" novalidate>
      ...
      <div x-show="currentStep === 1" x-transition class="glass-card rounded-xl p-6 space-y-6">
        <div>
          ...
          <p class="text-sm text-muted-foreground">Let's start with the quest basics</p>
        </div>

        {{-- Title --}}
        <div>
          <label for="title" class="block text-sm font-medium mb-2">Quest Title <span class="text-destructive">*</span></label>
          <input 
            type="text" 
            id="title" 
            name="title" 
        ...
        {{-- Description --}}
        <div>
          <label for="desc" class="block text-sm font-medium mb-2">Description <span class="text-destructive">*</span></label>
          <textarea 
            id="desc" 
            name="desc" 
            ...
            placeholder="Describe your quest, its purpose, and what participants will do..."
          >{{ old('desc') }}</textarea>
        ...
        {{-- Banner Upload --}}
        <div>
          <label for="banner" class="block text-sm font-medium mb-2">Quest Banner <span class="text-destructive">*</span></label>
          <div class="relative">
            <input 
              type="file" 
              id="banner" 
      ...
      <div x-show="currentStep === 2" x-transition class="glass-card rounded-xl p-6 space-y-6">
        <div>
          <h2 class="text-xl font-bold mb-1">Location & Timeline</h2>
          <p class="text-sm text-muted-foreground">Set where and when the quest will happen</p>
        </div>

        {{-- Location --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            <label for="location_name" class="block text-sm font-medium mb-2">Location Name <span class="text-destructive">*</span></label>
            <input 
              type="text" 
              id="location_name" 
              name="location_name" 
          ...
          <div class="md:col-span-2">
            ...
            <input 
              ...
              placeholder="100"
            >
            <p class="mt-1.5 text-xs text-muted-foreground">Participants must be within this radius to check in</p>
        ...
        {{-- Dates --}}
        <div class="space-y-4">
          <h3 class="font-semibold text-sm">Quest Timeline</h3>
          
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label for="registration_start_at" class="block text-sm font-medium mb-2">Registration Start <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
            ...
            <div>
              <label for="registration_end_at" class="block text-sm font-medium mb-2">Registration End <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
            ...
            <div>
              <label for="quest_start_at" class="block text-sm font-medium mb-2">Quest Start <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
                id="quest_start_at" 
                name="quest_start_at" 
            ...
            <div>
              <label for="quest_end_at" class="block text-sm font-medium mb-2">Quest End <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
                id="quest_end_at" 
                name="quest_end_at" 
            ...
            <div>
              <label for="judging_start_at" class="block text-sm font-medium mb-2">Judging Start <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
                id="judging_start_at" 
            ...
            <div>
              <label for="judging_end_at" class="block text-sm font-medium mb-2">Judging End <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
                id="judging_end_at" 
                name="judging_end_at" 
            ...
            <div class="md:col-span-2">
              <label for="prize_distribution_date" class="block text-sm font-medium mb-2">Prize Distribution Date <span class="text-destructive">*</span></label>
              <input 
                type="datetime-local" 
      ...
      <div x-show="currentStep === 3" x-transition class="glass-card rounded-xl p-6 space-y-6">
        <div>
          <h2 class="text-xl font-bold mb-1">Participant Settings</h2>
          <p class="text-sm text-muted-foreground">Set limits for participants and winners</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label for="participant_limit" class="block text-sm font-medium mb-2">Participant Limit <span class="text-destructive">*</span></label>
            <input 
              type="number" 
              ...
              placeholder="100"
            >
            <p class="mt-1.5 text-xs text-muted-foreground">Maximum number of participants allowed</p>
          ...
          <div>
            <label for="winner_limit" class="block text-sm font-medium mb-2">Winner Limit <span class="text-destructive">*</span></label>
            <input 
              type="number" 
              id="winner_limit" 
              name="winner_limit" 
              ...
              placeholder="10"
            >
            <p class="mt-1.5 text-xs text-muted-foreground">Maximum number of winners who will receive prizes</p>
        ...
        <div>
          <label for="liveness_code" class="block text-sm font-medium mb-2">Liveness Code (Optional)</label>
          <input 
            type="text" 
            id="liveness_code" 
            name="liveness_code" 
            ...
            placeholder="Enter a secret code for verification"
          >
          <p class="mt-1.5 text-xs text-muted-foreground">A code that participants need to enter during the quest for verification</p>
          @error('liveness_code')
            <p class="mt-1.5 text-sm text-destructive">{{ $message }}</p>
          @enderror
        </div>
      ...
      <div x-show="currentStep === 4" x-transition class="glass-card rounded-xl p-6 space-y-6">
        <div>
          <h2 class="text-xl font-bold mb-1">Quest Prizes</h2>
          <p class="text-sm text-muted-foreground">Define the rewards for quest winners</p>
        </div>
        ...
        <div class="p-4 bg-primary/5 border border-primary/20 rounded-xl space-y-4">
          <div class="flex items-center gap-2 mb-2">
            ...
            <h3 class="font-semibold">Certificate (Required)</h3>
          </div>

          <div>
            <label for="cert_name" class="block text-sm font-medium mb-2">Certificate Name <span class="text-destructive">*</span></label>
            <input 
              type="text" 
              id="cert_name" 
              name="cert_name" 
          ...
          <div>
            <label for="cert_description" class="block text-sm font-medium mb-2">Certificate Description <span class="text-destructive">*</span></label>
            <textarea 
              id="cert_description" 
          ...
          <div>
            <label for="cert_image" class="block text-sm font-medium mb-2">Certificate Image <span class="text-destructive">*</span></label>
            <input 
              type="file" 
              id="cert_image" 
              name="cert_image" 
            ...
            >
              <div x-show="!certImagePreview" class="w-full h-full flex flex-col items-center justify-center gap-3">
                ...
                <div class="text-center">
                  <p class="text-sm font-medium">Click to upload certificate template</p>
                  <p class="text-xs text-muted-foreground mt-1">PNG, JPG up to 2MB</p>
                </div>
              </div>
        ...
        <div class="p-4 bg-accent/5 border border-accent/20 rounded-xl space-y-4">
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-semibold">Coupon (Optional)</h3>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              >
              <span class="text-sm">Add coupon prize</span>
            </label>
          </div>
          ...
          <div x-show="hasCoupon" x-transition class="space-y-4">
            ...
            <div>
              <label for="coupon_description" class="block text-sm font-medium mb-2">Coupon Description</label>
              <textarea 
            ...
            <div>
              <label for="coupon_image" class="block text-sm font-medium mb-2">Coupon Image</label>
              <input 
                type="file" 
                id="coupon_image" 
                name="coupon_image" 
              ...
              >
                <div x-show="!couponImagePreview" class="w-full h-full flex flex-col items-center justify-center gap-3">
                  ...
                  <div class="text-center">
                    <p class="text-sm font-medium">Click to upload coupon image</p>
                    <p class="text-xs text-muted-foreground mt-1">PNG, JPG up to 2MB</p>
                  </div>
                </div>
