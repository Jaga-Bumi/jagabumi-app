...
<div class="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 py-8">
    <div class="container mx-auto px-4">
        ...
        <div class="glass-card p-6 rounded-2xl">
            ...
            @php
                // Assuming your User model has a 'articles' relationship
                $userArticles = auth()->user()->articles()->latest()->take(5)->get();
            @endphp

            @if($userArticles->count() > 0)
                <div class="space-y-4">
                    @foreach($userArticles as $article)
                        <div class="flex justify-between items-center p-4 rounded-xl bg-muted/50 hover:bg-muted transition-colors">
                            <div class="flex-1 min-w-0 pr-4">
                                <h3 class="font-semibold text-foreground truncate">{{ $article->title }}</h3>
                                <p class="text-sm text-muted-foreground">
                                    Published: {{ $article->created_at->format('M j, Y') }}
                                </p>
                            </div>
