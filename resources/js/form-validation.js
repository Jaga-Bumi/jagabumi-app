    ...
    validateField(fieldName) {
        ...
        if (isValid && fieldRules.max && value.length > fieldRules.max) {
            ...
            errorMessage =
                ...
                `${this.formatFieldName(fieldName)} cannot exceed ${
                    fieldRules.max
                } characters.`;
        }
        ...
        if (isValid && fieldRules.url && value) {
            try {
                ...
                isValid = false;
                errorMessage =
                    fieldRules.messages?.url || "Please enter a valid URL.";
            }
        }
        ...
        if (
            ...
            fieldRules.in &&
            value &&
            !fieldRules.in.includes(value)
        ) {
            isValid = false;
            errorMessage =
                fieldRules.messages?.in ||
                `Please select a valid ${this.formatFieldName(fieldName)}.`;
        }
        ...
        if (isValid && fieldRules.file && field.files && field.files[0]) {
            ...
            if (fieldRules.mimes) {
                ...
                if (!fieldRules.mimes.includes(extension)) {
                    isValid = false;
                    errorMessage =
                        fieldRules.messages?.mimes ||
                        `File must be one of: ${fieldRules.mimes.join(", ")}`;
                }
            }
            ...
            if (fieldRules.maxSize && file.size > fieldRules.maxSize * 1024) {
                isValid = false;
                errorMessage =
                    fieldRules.messages?.maxSize ||
                    `File size cannot exceed ${fieldRules.maxSize}KB`;
            }
            ...
            if (fieldRules.image && !file.type.startsWith("image/")) {
                isValid = false;
                errorMessage =
                    fieldRules.messages?.image || "File must be an image.";
            }
        }
